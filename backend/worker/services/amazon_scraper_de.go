...
//Edge Cases:
//1. Product not found
//2. Product uses subscription
//3. Normal Product
//4. Product uses rates B077JBQZPX
	

//AmazonProductEn is the default product struct for ASIN service
type AmazonProductDe struct {
...
func (product *AmazonProductDe) GetProductInfoByASIN() (res *colly.Response, err error) {
	//ToDo: Take domain as a request for Phase 2
	domain := "www.amazon.de"
	var productURL = "https://" + domain + "/dp/" + product.Asin
	// Instantiate default collector
	fmt.Println("Scraping: ", productURL)
	...
	cookie := c.Cookies(productURL)
	err = c.SetCookies(productURL, cookie)
	if err != nil {
		return
	}
	...
	*/
	...
	//Price string for product with monthly subscription
	c.OnHTML(".a-text-price.a-text-price.header-price",
		func(e *colly.HTMLElement) {
			price := e.ChildText(".a-offscreen")
			fmt.Println(price)
		})

	c.OnHTML(".a-price.aok-align-center",
		func(e *colly.HTMLElement) {
			fmt.Println(e.Text)
		})
	c.Visit(productURL)
